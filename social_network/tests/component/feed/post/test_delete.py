...
def test_success(api_client, request_url, auth_user1):
    ...
    response = api_client.delete(request_url)

    assert response.status_code == HTTPStatus.NO_CONTENT


@pytest.mark.django_db
def test_delete_other_post(api_client, request_url, auth_user1):
    api_client.force_authenticate(user=auth_user1)

    response = api_client.delete("/api/feed/feed/2/")

    assert response.status_code == HTTPStatus.FORBIDDEN


@pytest.mark.django_db
def test_delete_not_found(api_client, request_url, auth_user1):
    api_client.force_authenticate(user=auth_user1)

    response = api_client.delete("/api/feed/feed/3333/")
